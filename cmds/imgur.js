...
export default {
  ...
  execute: async (client, message, args) => {
    ...
    if (!mediaUrl) return client.sendMessage(chatId, "❌ No media found.");

    try {
      const endpoint = "https://azadx69x.is-a.dev/api/imgur?url=" + encodeURIComponent(mediaUrl);
      const response = await axios.get(endpoint, { timeout: 20000 });
      const data = response.data;
      if (!data || !data.url) return client.sendMessage(chatId, "❌ Upload failed.");
      client.sendMessage(chatId, data.url);
    } catch {
      client.sendMessage(chatId, "❌ Error uploading.");
    }
  }
};
